...
export const TemplateCNMei: React.FC<{ data: ResumeData; styles: any; disableShadow?: boolean }> = ({ data, styles, disableShadow }) => {
  const { t } = useLanguage();
  const getSectionTitle = useSectionTitle();
  const accent = data.themeConfig?.color || '#7c3aed';
  return (
      ...
      <div className="p-10">
        ...
        <div className="space-y-8">
          {(data.sections || []).filter(s => s.isVisible).map(section => (
            <div key={section.id}>
              <h3 className="text-lg font-semibold mb-4 inline-flex items-center gap-2" style={{ color: accent }}>
              ...
              {section.type === ResumeSectionType.Skills ? (
                <div className="flex flex-wrap gap-2">
                  {section.items.map(item => (
                    <span key={item.id} className="px-3 py-1 bg-purple-50 text-purple-900 rounded border" style={{ borderColor: accent }}>{item.description}</span>
                  ))}
                </div>
              ) : (
