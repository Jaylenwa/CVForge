...
export const TemplateCNLedger: React.FC<{ data: ResumeData; styles: any; disableShadow?: boolean }> = ({ data, styles, disableShadow }) => {
  const { t } = useLanguage();
  const getSectionTitle = useSectionTitle();
  const accent = data.themeConfig?.color || '#374151';
  return (
        ...
        <div className="border-b pb-6 mb-6" style={{ borderColor: '#e5e7eb' }}>
          <div className="flex justify-between items-center">
            ...
            {data.personalInfo.avatarUrl && (
              <img src={data.personalInfo.avatarUrl} alt={t('a11y.avatarAlt')} className="w-24 h-24 rounded object-cover border" style={{ borderColor: accent }} />
            )}
          </div>
        </div>
          ...
          {(data.sections || []).filter(s => s.isVisible).map(section => (
            <div key={section.id}>
              <div className="flex items-center mb-4">
                ...
                <h3 className="px-3 text-sm font-bold uppercase tracking-widest" style={{ color: accent }}>{getSectionTitle(section)}</h3>
                <div className="flex-1 h-px bg-gray-200"></div>
              </div>
              {section.type === ResumeSectionType.Skills ? (
                ...
                <div className="space-y-6">
                  {section.items.map(item => (
                    <div key={item.id} className="border-l pl-4" style={{ borderColor: '#e5e7eb' }}>
