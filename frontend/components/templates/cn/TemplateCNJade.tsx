...
export const TemplateCNJade: React.FC<{ data: ResumeData; styles: any; disableShadow?: boolean }> = ({ data, styles, disableShadow }) => {
  const { t } = useLanguage();
  const getSectionTitle = useSectionTitle();
  const accent = data.themeConfig?.color || '#10b981';
  return (
      ...
      <div className="p-10">
        <div className="flex justify-between items-center mb-8">
          ...
          {data.personalInfo.avatarUrl && (
            <img src={data.personalInfo.avatarUrl} alt={t('a11y.avatarAlt')} className="w-24 h-24 rounded-md object-cover border" style={{ borderColor: accent }} />
          )}
        </div>
        ...
        <div className="space-y-8">
          {(data.sections || []).filter(s => s.isVisible).map(section => (
            <div key={section.id}>
              <h3 className="text-lg font-semibold mb-4 pl-3 border-l-4" style={{ borderColor: accent, color: accent }}>{getSectionTitle(section)}</h3>
              {section.type === ResumeSectionType.Skills ? (
                <div className="flex flex-wrap gap-2">
                  {section.items.map(item => (
                    <span key={item.id} className="px-3 py-1 bg-emerald-50 text-emerald-900 rounded border" style={{ borderColor: accent }}>{item.description}</span>
                  ))}
                </div>
              ) : (
