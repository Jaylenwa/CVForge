...
export const TemplateCNSong: React.FC<{ data: ResumeData; styles: any; disableShadow?: boolean }> = ({ data, styles, disableShadow }) => {
  const { t } = useLanguage();
  const getSectionTitle = useSectionTitle();
  const accent = data.themeConfig?.color || '#1f2937';
  return (
      ...
      <div className="p-12">
        ...
        <div className="space-y-8">
          {(data.sections || []).filter(s => s.isVisible).map(section => (
            <div key={section.id}>
              <h3 className="text-lg font-bold uppercase tracking-widest mb-4 border-b pb-2" style={{ borderColor: accent, color: accent }}>{getSectionTitle(section)}</h3>
