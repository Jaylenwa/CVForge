...
export const TemplateClassic: React.FC<{ data: ResumeData; styles: any; disableShadow?: boolean }> = ({ data, styles, disableShadow }) => {
  ...
  const { spacingMode, lineHeight } = getSpacingTokens(styles);
  ...
  const jobMarginTopClassName = spacingMode === 'compact' ? 'mt-1' : spacingMode === 'spacious' ? 'mt-3' : 'mt-2';
  const headerInfoGridClassName =
    spacingMode === 'compact'
      ? 'mt-3 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1 text-gray-600 text-left'
      : spacingMode === 'spacious'
        ? 'mt-5 grid grid-cols-1 sm:grid-cols-2 gap-x-10 gap-y-3 text-gray-600 text-left'
        : 'mt-4 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-gray-600 text-left';
  ...
  <div className="pb-6 mb-6 flex flex-col md:flex-row items-center md:items-start gap-6">
      <div className="flex-1 text-center md:text-left order-2 md:order-1">
          ...
          {personal?.Job && <p className={`${jobMarginTopClassName} text-gray-600`}>{personal.Job}</p>}
