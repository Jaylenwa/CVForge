...
func (h *AdminHandler) AdminSeed(c *gin.Context) {
	...
	for _, item := range body {
		...
		if strings.TrimSpace(names["zh"]) == "" && strings.TrimSpace(item.Name) != "" {
			names["zh"] = item.Name
		}
		if strings.TrimSpace(pickName(names, "zh")) == "" && strings.TrimSpace(pickName(names, "en")) == "" {
			continue
		}
		items = append(items, SeedTemplateItem{ExternalID: item.ExternalID, Names: names})
	}

	if err := h.svc.Seed(items); err != nil {
